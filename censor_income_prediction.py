#This code describes the prediction if an adult earns more or less than 50,000 a year. It is a binary classification problem
...
features = ['age','workclass','fnlgwt','education','education-num','marital_status','occupation','relationship',
...
print('Duplicated Rows:', df_dup.shape)
# As seen in the output, there are 12 duplicated rows needed to be removed
# Remove duplicated rows
df = df.drop_duplicates()
print('New Data Shape after removing duplicate rows:',df.shape)
#print(df)
# Through inspection, the bank marketing dataset a multivariate dataset, having continuous,
# and categorical data along with missing values and suffers from outliers.

# Check country labels
country = df['native_country'].unique()
print('Countries:', country)


#Remove redundant columns that do not contribute to the target prediction
# From the data, fnlgwt and country do not have significant importance in prediction. Also, most attribute values of
# capital_gain and capital_gain are zero. They should be dropped
df = df.drop(['fnlgwt','capital_gain','capital_loss','native_country'], axis=1)
print(df.head(50).to_string())

# Check class labels in the data
df_class_labels = df['income'].unique()
print(df_class_labels)

# Income (Class label) distribution among samples
class_dist = df['income'].value_counts()
print(class_dist)


# Check missing values
print('Replacing ? with default Nan in Numpy')
...
# Replace Missing Values: Delete columns or rows having missing values more than 30% or to input values if less--------
...
y = df['income'].copy()
print(y.head(50).to_string())


# Transform the categorical variables to numeric values
trans = ce.OneHotEncoder(cols=['workclass', 'education', 'marital_status', 'occupation',
...
for i in df:
    print(df[i].describe())
# From the statistical summary, the spread of the data (standard deviation) is seen to be far from the mean
# for some features. Standardization or Normalization is needed
...
print('<=$50K:',grp_data[0])
print('>$50K:',grp_data[1])
...
print('<=$50K:',sum(y_train==0))
print('>$50K:',sum(y_train==1))
...
#plt.show()
plt.savefig('Censor_Hyperparameter_Output')
print('\n')
