...
data = "C:/Users/Gabriel/Desktop/bc_data.csv"
features = ['Class','age','menopause','tumor-size','inv-nodes','node-caps','deg-malig','breast',
            'breast-quad','irradiat']
df = pd.read_csv(data, delimiter=',',names=features)
# Inspect data
#print(df.to_string())
# Through inspection, the breast cancer dataset is a categorical data that contains missing values and class
# imbalance problem
# DATA PREPARATION--------------------------------------------------------
# Check data shape
...
df_bef_rem = df.isnull().sum()
print('Before Imputting: Missing values in each feature \n:-------------------------------')
print(df_bef_rem)
...
# Density plots
X.plot(kind='density', subplots=True, layout=(8,8), sharex=False, legend=False, fontsize=1, figsize=(12,12))
plt.show()
...
# Encoding attributes or Label Encoding: Transform the labels 'no_recurrent' and 'recurrent' to numeric binary values
...
print('No recurrence:',grp_data[0])
print('recurrence:',grp_data[1])
...
print('No recurrence:',sum(y_train==0))
print('recurrence:',sum(y_train==1))
...
scale = MinMaxScaler()#StandardScaler() # The robust scaler standardize the data and also deals with outliers in the data
...
#plt.show()
plt.savefig('Breast_Cancer_Hyperparameter_Output')
print('\n')
